...
public class DangerousPermissionRequest
{
	...
	public void enqueue(final DangerousPermResponseCallBack callback)
	{
		...
		if (!DangerousPermissionUtils.hasPermission(context, permissions, granted))
		{
			...
			{
				DangerousPermissionUtils.startPermissionRequest(context, permissions, requestCode, new ResultReceiver(new Handler())
				{
					@Override
					protected void onReceiveResult(int resultCode, Bundle resultData)
					{
						...
						String[] permissions = resultData.getStringArray(DangerousPermissionConstants.PERMISSIONS_ARRAY);
						response = new DangerousPermissionResponse(permissions, grantResult, resultCode);
						callback.onComplete(new DangerousPermissionResponse(permissions, grantResult, resultCode));
					}
				});
			}
			else
