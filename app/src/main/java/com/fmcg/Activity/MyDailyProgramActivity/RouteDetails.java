package com.fmcg.Activity.MyDailyProgramActivity;
				...
				for (int i = 0; i < stList.size(); i++)
				{
					...
					if (checkedStatus.isSelected() == true)
					{
						...
						if (selectedzoneName != null && !selectedzoneName.isEmpty())
						{
							...
							SharedPrefsUtil.setStringPreference(mContext, "SELECTED_ROUTEID",
							                                    String.valueOf(checkedStatus.getRouteId()));
						}
					}
					else
					{
...
//								routeNoSpinnerAdapter(jsonArray);
				...
				else if (response.getTag().equals("acceptRoute"))
				{
					try
					{
						if (mJson.getString("Message") != null
								&& !mJson.getString("Message").equals("null") && !mJson.getString("Message").isEmpty()
								)
						{
...
//							isChecked = false;
	...
	private void adapterAssigning(ArrayList<RouteDetailsData> _routeDetailsData)
	{
		if (_routeDetailsData.size() != 0)
		{
			...
			mAdapter = new RouteCheckListAdapter(this, _routeDetailsData);
			...
			if (stList.size() != 0)
			{
				for (int i = 0; i < stList.size(); i++)
				{
					...
					if (singleStudent.isSelected() == true)
					{
						...
						data = data + "\n" + String.valueOf(singleStudent.getRouteId()).toString();
						employeeRoutId = String.valueOf(singleStudent.getRouteId());
						selectedrouteNoDb.put(String.valueOf(singleStudent.getRouteId()).toString());
						SharedPrefsUtil.setStringPreference(mContext, "PLAN_STARTED", "ACCEPTED");
					}
					else
					{
				...
				if (employeeRoutId != null && !employeeRoutId.isEmpty())
				{
					String selcetdRoutesStr = selectedrouteNoDb.toString();
